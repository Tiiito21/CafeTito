...
const MenuSection = ({ section, dishes }: any) => {
  ...
  return (
    Object.entries(carta).map(([section, dishes]) => (
      <section key={section} className="mt-2 w-[90%] md:w-3/4 flex flex-col items-center cursor-pointer" onClick={() => toggleOpen(section)}>
        ...
        
        {
          ...
          {
            dishes.map((dish: any) => (   
              <li key={dish.nombre} className="text-md md:text-xl text-center font-serif font-bold text-secondary flex justify-between">
